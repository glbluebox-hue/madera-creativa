...
async function requireAuth(req, res, next) {
    ...
    try {
        ...
        const u = await UsuarioModel.findOne({ id: usuarioId }).lean().exec();
...
async function getPushSubsAdmin() {
    await conectarUsuarios();
    const admin = await UsuarioModel.findOne({ esAdmin: true }).lean().exec();
    return (admin?.pushSubs || []);
}
/** Notifica al admin cuando un nuevo usuario se registra. */
async function notificarAdminNuevoUsuario(nombre) {
    const subs = await getPushSubsAdmin();
    for (const sub of subs) {
        await enviarNotificacion(sub, 'Nuevo usuario pendiente', nombre + ' quiere acceder a la app. Entra para aprobar o rechazar.', { tipo: 'nuevo-usuario', nombre });
    }
}
 ...
 */
async function migrarDatosAdmin() {
    try {
        ...
        if (total > 0)
            console.log(`Migración admin: ${c?.modifiedCount} clientes, ${f?.modifiedCount} facturas, ${e?.modifiedCount} empresa`);
    }
    catch (err) {
...
export function run() {
     ...
     */
    app.get('/auth/yo', requireAuth, (req, res) => {
    ...
    app.post('/push/subscribe', async (req, res) => {
        try {
            ...
            const yaExiste = (u.pushSubs || []).some((s) => s.endpoint === subscription.endpoint);
            if (!yaExiste) {
    ...
    app.post('/auth/login', async (req, res) => {
        try {
            ...
            const u = await UsuarioModel.findOne({ nombre: { $regex: new RegExp('^' + nombre + '$', 'i') } }).lean().exec();
            if (!u || u.passwordHash !== passwordHash) {
                res.status(401).json({ error: 'Usuario o contraseña incorrectos.' });
                return;
            }
            if (u.estado === 'pendiente') {
                res.status(403).json({ error: 'pendiente', mensaje: 'Tu cuenta está pendiente de aprobación. Recibirás acceso en breve.' });
                return;
            }
            if (u.estado === 'suspendido') {
                res.status(403).json({ error: 'suspendido', mensaje: 'Tu acceso ha sido suspendido. Contacta con Madera Creativa.' });
                return;
            }
    ...
    app.post('/auth/verificar', async (req, res) => {
        try {
            ...
            const u = await UsuarioModel.findOne({ id: usuarioId }).lean().exec();
            if (!u) {
                res.status(404).json({ activo: false });
                return;
            }
    ...
    app.put('/admin/usuarios/:id/estado', requireAuth, async (req, res) => {
        try {
            ...
            const u = await UsuarioModel.findOneAndUpdate({ id: req.params.id }, { estado }, { new: true }).lean().exec();
            if (!u) {
    ...
    app.get('/clientes', requireAuth, async (req, res) => {
        try {
            ...
            const slim = clientes.map((c) => ({
                ...c,
                adjuntos: (c.adjuntos || []).map(({ url: _url, ...rest }) => rest),
            }));
            res.json(slim);
        }
        catch (err) {
    ...
    app.get('/empresa', requireAuth, async (req, res) => {
        try {
            res.json(await svc.obtenerEmpresa(req.usuarioId));
        }
        catch (err) {
            res.status(500).json({ error: String(err) });
        }
    });
    app.put('/empresa', requireAuth, async (req, res) => {
        try {
            res.json(await svc.guardarEmpresa(req.body, req.usuarioId));
        }
        catch (err) {
            res.status(500).json({ error: String(err) });
        }
    ...
    app.get('/facturas', requireAuth, async (req, res) => {
        try {
            res.json(await svc.listarFacturas(req.usuarioId));
        }
        catch (err) {
            res.status(500).json({ error: String(err) });
        }
    ...
    app.put('/facturas/:id', requireAuth, async (req, res) => {
        try {
            res.json(await svc.guardarFactura({ ...req.body, id: req.params.id }, req.usuarioId));
        }
        catch (err) {
            res.status(500).json({ error: String(err) });
        }
    ...
    app.delete('/facturas/:id', requireAuth, async (req, res) => {
        try {
            await svc.borrarFactura(req.params.id, req.usuarioId);
            res.json({ ok: true });
        }
        catch (err) {
            res.status(500).json({ error: String(err) });
    ...
    // ── Asistente IA ──
    app.post('/asistente', requireAuth, async (req, res) => {
        try {
            ...
            const ingresosMes = facturas.filter(f => f.tipo === 'ingreso' && f.fecha?.startsWith(mesActual)).reduce((s, f) => s + (f.importe || 0), 0);
            const gastosMes = facturas.filter(f => f.tipo === 'gasto' && f.fecha?.startsWith(mesActual)).reduce((s, f) => s + (f.importe || 0), 0);
...
//# sourceMappingURL=presupuestos-service.app-root.js.map