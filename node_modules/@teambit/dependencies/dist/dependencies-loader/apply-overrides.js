...
class ApplyOverrides {
  constructor(component, depsResolver, logger, workspace) {
   ...
   * @returns
   */
  updateVersionOfMissingsInEnvJsonc(deps) {
  ...
  _resolvePackageData(packageName) {
    ...
    const modulePath = (0, _legacy3().resolvePackagePath)(packageName, basePath, consumerPath);
    if (!modulePath) return undefined; // e.g. it's author and wasn't exported yet, so there's no node_modules of that component
    const packageObject = (0, _resolvePkgData().resolvePackageData)(basePath, modulePath);
  ...
  getDependenciesToAddManually(packageJson, existingDependencies) {
    ...
    const components = {};
    const packages = {};
    const missingPkgRemovals = new Set();
      ...
      Object.keys(overrides[depField]).forEach(dependency => {
        ...
        if (componentData?.componentId) {
          if (componentData.componentId.isEqualWithoutVersion(this.componentId)) {
            this.logger.warn(`component ${this.componentId.toString()} depends on itself ${componentData.componentId.toString()}. ignoring it.`);
            return;
          }
        ...
        this.allPackagesDependencies.packageDependencies[dependency] || this.allPackagesDependencies.devPackageDependencies[dependency] || this.allPackagesDependencies.peerPackageDependencies[dependency]) {
          missingPkgRemovals.add(dependency);
        }
      });
    });
    if (missingPkgRemovals.size > 0) {
      this.overridesDependencies.missingPackageDependencies = this.overridesDependencies.missingPackageDependencies.filter(pkg => !missingPkgRemovals.has(pkg));
    }
    return {
      components,
      packages
  ...
  manuallyAddDependencies() {
    ...
    _legacy().DEPENDENCIES_FIELDS.forEach(depField => {
      const otherFields = _legacy().DEPENDENCIES_FIELDS.filter(f => f !== depField);
      if (components[depField] && components[depField].length) {
        const depsSet = new Set();
        components[depField].forEach(depData => {
          depsSet.add(depData.packageName);
          this.allDependencies[depField].push(new (_legacy2().Dependency)(depData.componentId, [], depData.packageName, depData.versionRange));
        });
        otherFields.forEach(otherField => {
          this.allDependencies[otherField] = this.allDependencies[otherField].filter(dep => !depsSet.has(dep.packageName));
        });
      }
   ...
   */
  removeCompDepsFromPackages() {
    const currPackagesKeys = ['packageDependencies', 'devPackageDependencies', 'peerPackageDependencies'].reduce((acc, field) => {
  ...
  getMissingPackagesFromIssues() {
    const missingData = this.issues.getIssueByName('MissingPackagesDependenciesOnFs')?.data;
    if (!missingData) return [];
    return (0, _lodash().uniq)(missingData.map(d => d.missingPackages).flat());
  }
  isPackageAutoDetected(pkgName, originallyExists, missingPackages) {
    // Check component dependencies
    const existsInAnyCompDeps = this.allDependencies.dependencies.some(dep => dep.packageName === pkgName) || this.allDependencies.devDependencies.some(dep => dep.packageName === pkgName) || this.allDependencies.peerDependencies.some(dep => dep.packageName === pkgName);
    ...
    const existsInOriginalPkgDeps = this.originAllPackagesDependencies.packageDependencies[pkgName] || this.originAllPackagesDependencies.devPackageDependencies[pkgName] || this.originAllPackagesDependencies.peerPackageDependencies[pkgName];
    ...
    return existsInAnyCompDeps || !!existsInOriginalPkgDeps || originallyExists.includes(pkgName) || missingPackages.includes(pkgName);
  }
  applyAutoDetectedOverrideForPackage(field, pkgName, pkgVal, originallyExists, missingPackages) {
   ...
   */
  handlePeerDependencyOverride(pkgName, existsInCompsDeps, existsInCompsDevDeps) {
  ...
  removePackageFromAllDependencyTypes(pkgName) {
    delete this.allPackagesDependencies.packageDependencies[pkgName];
    delete this.allPackagesDependencies.devPackageDependencies[pkgName];
    delete this.allPackagesDependencies.peerPackageDependencies[pkgName];
  }
  applyPackageVersion(field, pkgName, pkgVal, existsInCompsDeps, existsInCompsDevDeps, existsInCompsPeerDeps) {
    ...
    const shouldAddPackage = pkgVal !== _legacy().MANUALLY_REMOVE_DEPENDENCY && (!existsInCompsDeps && !existsInCompsDevDeps || field === 'peerDependencies');
  ...
  async applyAutoDetectedPeersFromEnvOnEnvItSelf() {
    ...
    this.allPackagesDependencies.packageDependencies = _objectSpread(_objectSpread({}, deps), resolvedEnvPolicyManifest);
  }
   ...
   */
  resolveEnvPeerDepVersion(pkgName) {
  ...
  _getPackageJsonFromComponentModel() {
    if (this.componentFromModel && this.component.componentMap) {
      ...
      const packageJson = _component().PackageJsonFile.createFromComponent(this.component.componentMap.rootDir, this.componentFromModel);
