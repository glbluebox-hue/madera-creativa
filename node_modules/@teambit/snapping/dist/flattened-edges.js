...
exports.FlattenedEdgesGetter = void 0;
function _graph() {
 ...
 */
class FlattenedEdgesGetter {
  constructor(scope, consumerComponents, logger, lane) {
    this.scope = scope;
    this.consumerComponents = consumerComponents;
    this.logger = logger;
    this.lane = lane;
    _defineProperty(this, "graph", new (_graph().Graph)());
    _defineProperty(this, "missingFromGraph", []);
  }
  async buildGraph() {
    ...
    this.logger.debug(`FlattenedEdgesGetter, total ${this.missingFromGraph.length} components missing from graph, trying to find them in previous versions`);
    await this.addPreviousGraphs();
    if (!this.missingFromGraph.length) {
      this.logger.debug(`FlattenedEdgesGetter, all missing ids were found in previous versions`);
      return this.graph;
    }
    this.logger.debug(`FlattenedEdgesGetter, total ${this.missingFromGraph.length} components missing from graph, trying to import them and load their flattenedEdges`);
    ...
    this.logger.debug(`FlattenedEdgesGetter, total ${this.missingFromGraph.length} components without graph and their version-objects has no flattened edges, no choice but to import all their flattened deps`);
    await this.addComponentsWithMissingFlattenedEdges();
    return this.graph;
  }
  populateFlattenedAndEdgesForComp(component) {
    const graphFromIds = this.graph.successorsSubgraph(component.id.toString());
    const edgesFromGraph = graphFromIds.edges.map(edge => {
      return {
        source: _componentId().ComponentID.fromString(edge.sourceId),
        target: _componentId().ComponentID.fromString(edge.targetId),
        type: edge.attr
      };
    });
    ...
    component.flattenedDependencies = _componentId().ComponentIdList.fromArray(flattenedFromGraph);
    component.flattenedEdges = edgesFromGraph;
  }
  async importMissingAndAddToGraph() {
    const idsWithoutGraphList = _componentId().ComponentIdList.fromArray(this.missingFromGraph);
    ...
    const missingEdges = [];
    await Promise.all(componentsAndVersions.map(async ({
      component,
      version,
      versionStr
    }) => {
  ...
  async addPreviousGraphs() {
    ...
    const flattenedDepsUniq = _componentId().ComponentIdList.uniqFromArray(flattenedDeps.flat());
    this.missingFromGraph = this.missingFromGraph.filter(id => !flattenedDepsUniq.has(id));
  }
  async addComponentsWithMissingFlattenedEdges() {
...
${missingEdges.map(e => e.toString()).join('\n')}`);
    const missingEdgesList = _componentId().ComponentIdList.fromArray(missingEdges);
    ...
    const componentsAndVersions = await this.scope.legacyScope.getComponentsAndVersions(_componentId().ComponentIdList.fromArray(allFlattenedUniq));
    componentsAndVersions.forEach(({
      ...
      this.graph.setNode(new (_graph().Node)(compId.toString(), compId));
  ...
  addFlattenedEdgesToGraph(flattenedEdges) {
    flattenedEdges.forEach(({
      source,
      target,
      type
    }) => {
      this.graph.setNode(new (_graph().Node)(source.toString(), source));
      this.graph.setNode(new (_graph().Node)(target.toString(), target));
      this.graph.setEdge(new (_graph().Edge)(source.toString(), target.toString(), type));
    });
  }
  populateMissingFromGraph() {
    const allIds = this.graph.nodes.map(node => node.attr);
    const currentlySnappedIds = _componentId().ComponentIdList.fromArray(this.consumerComponents.map(comp => comp.id));
  ...
  buildTheFirstLevel() {
    this.consumerComponents.forEach(comp => {
      const id = comp.id;
      this.graph.setNode(new (_graph().Node)(id.toString(), id));
  ...
  addEdges(compId, dependencies, label) {
    dependencies.get().forEach(dep => {
      this.graph.setNode(new (_graph().Node)(dep.id.toString(), dep.id));
      this.graph.setEdge(new (_graph().Edge)(compId.toString(), dep.id.toString(), label));
    });
  }
}
exports.FlattenedEdgesGetter = FlattenedEdgesGetter;
...
//# sourceMappingURL=flattened-edges.js.map