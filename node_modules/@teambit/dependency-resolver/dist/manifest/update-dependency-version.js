...
function updateDependencyVersion(dependency, rootPolicy, variantPolicy) {
  if (dependency.getPackageName) {
    ...
    const specialChars = ['-', '+', '*'];
    const variantVersionWithoutSpecialChar = variantVersion && !specialChars.includes(variantVersion) ? variantVersion : undefined;
    const version = variantVersionWithoutSpecialChar || rootPolicy?.getValidSemverDepVersion(packageName, dependency.lifecycle === 'peer' ? 'peer' : 'runtime') || (0, _componentPackageVersion().snapToSemver)(dependency.version) || '0.0.1-new';
