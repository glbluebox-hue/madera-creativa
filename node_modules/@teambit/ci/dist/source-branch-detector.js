...
class SourceBranchDetector {
   ...
   * ```
   */
  async getSourceBranchName(commitSha) {
    try {
   ...
   */
  async getSourceBranchFromGitHubAPI(commitSha) {
    try {
      ...
      const targetCommit = commitSha || (await _git().git.revparse(['HEAD']));
      if (!targetCommit) {
   ...
   */
  async getGitCommitMessage(commitSha) {
    try {
      const logOptions = {
        maxCount: 1
      };
      ...
      const commit = await _git().git.log(logOptions);
      if (!commit.latest) {
        return null;
      }
      const {
