...
exports.extractSkipTasksFromMessage = extractSkipTasksFromMessage;
/**
...
function extractSkipTasksFromMessage(message) {
  const match = message.match(SKIP_TASKS_TOKEN);
  if (!match) return {
    skipTasks: [],
    message
  };
  const skipTasks = match[1].split(',').map(task => task.trim()).filter(Boolean);
  const cleanedMessage = message.replace(match[0], '').replace(/[ \t]{2,}/g, ' ').trim();
  return {
    skipTasks,
...
//# sourceMappingURL=skip-tasks-from-message.js.map