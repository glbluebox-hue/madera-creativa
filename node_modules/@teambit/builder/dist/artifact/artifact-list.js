...
class ArtifactList extends Array {
  ...
  async getVinylsAndImportIfMissing(id, scope) {
    ...
    const vinyls = await (0, _pMapSeries().default)(this, artifact => artifact.files.getVinylsAndImportIfMissing(id, scope));
    return vinyls.flat();
  }
  ...
  async store(component) {
    ...
    await Promise.all(Object.keys(byResolvers).map(async key => {
      ...
      await Promise.all(artifactList.map(async artifact => {
