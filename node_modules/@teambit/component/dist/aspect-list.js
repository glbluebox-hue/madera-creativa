...
class AspectList {
  ...
  addEntry(aspectId, data = {}) {
    const extensionDataEntry = new (_legacy().ExtensionDataEntry)(undefined, aspectId, undefined, {}, data);
    const entry = new (_aspectEntry().AspectEntry)(aspectId, extensionDataEntry);
    this.entries.push(entry);
    return entry;
  }
  upsertEntry(aspectId, data = {}) {
  ...
  toConfigObject() {
    ...
    this.entries.forEach(entry => {
      if (entry.config) {
        res[entry.id.toString()] = (0, _legacy().removeInternalConfigFields)(entry.legacy.rawConfig);
      }
    });
    return res;
  }
  ...
  toLegacy() {
    ...
    return _legacy().ExtensionDataList.fromArray(legacyEntries);
  }
  stringIds() {
