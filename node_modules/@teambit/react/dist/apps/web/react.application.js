...
class ReactApp {
  constructor(name, entry, ssr, portRange, reactEnv, logger, dependencyResolver, bundler, ssrBundler, devServer, transformers = [], deploy, favicon, webpackModulePath, webpackDevServerModulePath) {
    this.name = name;
    this.entry = entry;
    this.ssr = ssr;
    ...
    _defineProperty(this, "applicationType", 'react-common-js');
    _defineProperty(this, "dir", 'public');
    _defineProperty(this, "ssrDir", 'ssr');
  }
  async run(context) {
    const [from, to] = this.portRange;
    const port = context.port || (await _toolboxNetwork().Port.getPort(from, to));
    if (this.devServer) {
    ...
    const devServer = this.reactEnv.getDevServer(devServerContext, [_mutators().addDevServer, _mutators().setOutput, _mutators().setDevServerClient, ...this.transformers], this.webpackModulePath, this.webpackDevServerModulePath);
  ...
  async buildClient(context) {
    ...
    const bundler = await this.reactEnv.getBundler(ctx, [config => config.merge([(0, _webpackAppSsr().clientConfig)()]), ...this.transformers], this.webpackModulePath);
  ...
  async buildSsr(context) {
    ...
    const bundler = await this.reactEnv.getBundler(ctx, [config => config.merge([(0, _webpackAppSsr().ssrConfig)()]), ...this.transformers], this.webpackModulePath);
    const bundleResult = await bundler.run();
    return bundleResult[0];
  }
  async build(context) {
  ...
  async getDefaultBundler(context) {
    ...
    })), (0, _mutators().replaceTerserPlugin)(), ...this.transformers]);
    const reactEnv = context.env;
    const bundler = await reactEnv.getBundler(bundlerContext, transformers, this.webpackModulePath);
    return bundler;
  }
