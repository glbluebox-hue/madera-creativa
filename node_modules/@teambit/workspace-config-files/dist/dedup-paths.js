...
exports.buildCompPathExtendingHashMap = buildCompPathExtendingHashMap;
...
function getAllPossibleDirsFromPaths(paths, componentsRootDir) {
  const dirs = paths.map(p => getAllParentsDirOfPath(p)).flat();
  const uniqueDirs = (0, _lodash().uniq)(dirs);
  ...
  return (0, _lodash().uniq)(filteredDirs);
}
function getAllParentsDirOfPath(p) {
  ...
  while (current !== '.') {
    all.push(current);
    current = (0, _path().dirname)(current);
  }
  return all;
}
function buildCompPathExtendingHashMap(extendingConfigFilesMap, envCompsDirsMap) {
  const map = Object.entries(extendingConfigFilesMap).reduce((acc, [hash, {
    envIds
  }]) => {
 ...
 */
function dedupePaths(extendingConfigFilesMap, envCompsDirsMap, componentsRootDir) {
  const rootDir = '.';
  ...
  const allPathsPerFileHash = {}; // null when parent-dir has same amount of comps per env.

  const calculateBestFileForDir = dir => {
    ...
    const countPerFileHash = {};
    allPathsShareSameDir.forEach(p => {
      const fileHash = compPathExtendingHashMap[p];
      if (countPerFileHash[fileHash]) countPerFileHash[fileHash] += 1;else countPerFileHash[fileHash] = 1;
    });
    ...
    if (fileHashWithMax.length > 1) allPathsPerFileHash[dir] = null;else allPathsPerFileHash[dir] = fileHashWithMax[0];
  };
  ...
  const dedupedPathsPerFileHash = Object.keys(allPathsPerFileHash).reduce((acc, current) => {
    if (allPathsPerFileHash[current] && allPathsPerFileHash[(0, _path().dirname)(current)] !== allPathsPerFileHash[current]) {
  ...
  const fileHashPerDedupedPaths = (0, _lodash().invertBy)(dedupedPathsPerFileHash);
...
//# sourceMappingURL=dedup-paths.js.map