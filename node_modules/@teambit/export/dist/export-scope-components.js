...
async function mergeObjects(scope, bitObjectList, throwForMissingDeps = false) {
  ...
  _legacy().logger.debug(`export-scope-components.mergeObjects, Going to merge ${components.length} components, ${lanesObjects.length} lanes, ${versions.length} versions, ${lanesHistory.length} lane histories`);
  const {
    mergeResults,
    errors
  } = lanesObjects.length ? {
