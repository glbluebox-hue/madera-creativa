...
exports.ServerCommander = void 0;
exports.shouldUseBitServer = shouldUseBitServer;
function _nodeFetch() {
...
Please run the command "bit server-forever" first to start the server.`));
  ...
  initSSE(url) {
    ...
    eventSource.addEventListener('onLoader', event => {
      const parsed = JSON.parse(event.data);
      const {
        method,
        args
      } = parsed;
      _legacy().loader[method](...(args || []));
    });
    eventSource.addEventListener('onLogWritten', event => {
      const parsed = JSON.parse(event.data);
  ...
  getServerPortFilePath() {
    ...
    return (0, _path().join)(scopePath, 'server-port.txt');
  }
}
