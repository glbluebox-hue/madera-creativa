...
function _toolboxJson() {
  const data = require("@teambit/toolbox.json.jsonc-utils");
  _toolboxJson = function () {
    return data;
  };
  return data;
}
...
class InstallMain {
  ...
  async _installModules(options) {
    ...
    const pmInstallOptions = _objectSpread(_objectSpread({}, calcManifestsOpts), {}, {
      ...
      overrides: _objectSpread(_objectSpread({}, current.peerOverrides), this.dependencyResolver.config.overrides),
      hoistPatterns: this.dependencyResolver.config.hoistPatterns,
    ...
    do {
      ...
      let installResult;
      try {
        installResult = await installer.installComponents(this.workspace.path, current.manifests, mergedRootPolicy, current.componentDirectoryMap, {
          linkedDependencies,
   ...
   */
  enrichUnpublishedSnapDepError(err, components) {
    ...
    const pnpmCode = err?.cause?.code ?? err?.code;
    const errMessage = err.message || '';
    const isNoMatchingVersion = pnpmCode === 'ERR_PNPM_NO_MATCHING_VERSION' || !pnpmCode && errMessage.includes('No matching version found');
    ...
    for (const component of components) {
      for (const dep of this.dependencyResolver.getComponentDependencies(component)) {
        ...
        if (!depId.version || !(0, _componentVersion().isSnap)(depId.version)) continue;
        ...
        if (entry) entry.dependents.push(dependent);else unpublished.set(key, {
          id: key,
    ...
    return new (_exceptions().UnpublishedComponentDependency)([...unpublished.values()]);
  }
  ...
  async _getComponentsManifests(dependencyInstaller, rootPolicy, installOptions) {
    ...
    const result = await dependencyInstaller.getComponentManifests(_objectSpread(_objectSpread({}, installOptions), {}, {
  ...
  async _getEnvManifests(workspaceDeps) {
    ...
    return Object.fromEntries(await Promise.all(envs.map(async envId => {
      return [await this.getRootComponentDirByRootId(this.workspace.rootComponentsPath, envId), {
        dependencies: _objectSpread(_objectSpread(_objectSpread({}, await this._getEnvDependencies(envId, workspaceDeps)), workspaceDeps), await this._getEnvPackage(envId)),
        installConfig: {
   ...
   */
  async _getEnvDependencies(envId, workspaceDeps) {
  ...
  async _getAppManifests(manifests, workspaceDeps) {
    return Object.fromEntries((0, _lodash().compact)(await Promise.all((await this.app.listAppsComponents()).map(async app => {
      ...
      return [await this.getRootComponentDirByRootId(this.workspace.rootComponentsPath, app.id), _objectSpread(_objectSpread({}, (0, _lodash().omit)(appManifest, ['name', 'version'])), {}, {
        dependencies: _objectSpread(_objectSpread(_objectSpread({}, await this._getEnvDependencies(envId, workspaceDeps)), appManifest.dependencies), workspaceDeps),
        installConfig: {
          hoistingLimits: 'workspaces'
        }
      })];
  ...
  async updateEnvJsoncPolicies(outdatedPkgs) {
    ...
    await Promise.all(Array.from(updatesByComponentId.values()).map(async pkgs => {
      ...
      const updatedContent = (0, _toolboxJson().updateJsoncPreservingFormatting)(envJsoncContent, envJsonc => {
        pkgs.forEach(pkg => {
          let field;
          ...
          const depEntry = deps.find(({
            name
          }) => name === pkg.name);
          if (depEntry) {
            depEntry.version = pkg.latestRange;
            if (field === 'peers' && depEntry.supportedRange) {
              if (!_semver().default.intersects(pkg.latestRange, depEntry.supportedRange)) {
      ...
      const absPath = _path().default.join(this.workspace.componentDir(component.id), 'env.jsonc');
      await _fsExtra().default.writeFile(absPath, updatedContent);
    }));
  }
  ...
  async updateDependencies(options) {
    ...
    const envJsoncUpdates = [];
    const policiesUpdates = [];
    ...
    const {
      updatedVariants,
      updatedComponents
    } = this.dependencyResolver.applyUpdates(policiesUpdates, {
      variantPoliciesByPatterns
    });
    await Promise.all([this.updateEnvJsoncPolicies(envJsoncUpdates), this._updateVariantsPolicies(updatedVariants), this._updateComponentsConfig(updatedComponents)]);
   ...
   */
  async syncCoreAspectLinksForEnvs(compDirMap) {
    if (this.dependencyResolver.linkCoreAspects()) return;
    const linker = this.dependencyResolver.getLinker({
      rootDir: this.workspace.path
    });
    await linker.syncCoreAspectLinksForEnvs(this.workspace.path, compDirMap.toArray().map(([component]) => component.id));
  }
  ...
  async removePostInstallScript() {
    try {
      ...
      if (packageJsonFile.packageJsonObject.scripts?.postinstall === _hostInitializer().EXTERNAL_PM_POSTINSTALL_SCRIPT) {
