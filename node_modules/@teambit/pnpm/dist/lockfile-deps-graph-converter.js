...
async function convertGraphToLockfile(_graph, {
  ...
  for (const edge of graph.edges) {
    if (edge.id === _objects().DependenciesGraph.ROOT_EDGE_ID) continue;
  ...
  const failedWorkspaceComponentPkgs = new Set();
  await Promise.all(pkgsToResolve.map(async pkgToResolve => {
    if (lockfile.packages[pkgToResolve.pkgId].resolution == null) {
      ...
      try {
        ...
        if (err.code === 'ERR_PNPM_NO_MATCHING_VERSION' && graph.packages.get(pkgToResolve.pkgId)?.component != null) {
  ...
  for (const [pkgId, pkg] of Object.entries(lockfile.packages)) {
    if ((pkg.resolution == null || Object.keys(pkg.resolution).length === 0) && graph.packages.get(pkgId)?.component != null) {
  ...
  for (const [depPath, pkg] of Object.entries(lockfile.packages)) {
    if (pkg.resolution == null || Object.keys(pkg.resolution).length === 0) {
      throw new (_bitError().BitError)(`Failed to generate a valid lockfile. The "packages['${depPath}']" entry doesn't have a "resolution" field.`);
    }
  }
  return lockfile;
...
function getPkgsToResolve(lockfile, manifests) {
  const pkgsToResolve = [];
  const pkgsInTheWorkspaces = new Map();
  for (const {
    name,
    version
  } of Object.values(manifests)) {
    if (name && version) {
