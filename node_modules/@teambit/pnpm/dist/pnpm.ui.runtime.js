...
class PnpmUI {
  ...
  constructor(compUI) {
    this.compUI = compUI;
    _defineProperty(this, "consumeMethod", ({
      id: componentId,
  ...
  getNpmConfig(registry, authToken) {
    const registryUrl = 'https://node-registry.bit.cloud';
    const configs = [`${registry}:registry="${registryUrl}"`, authToken ? `"//node-registry.bit.cloud/:_authToken=${authToken}"` : undefined].filter(Boolean);
    return `npm config set ${configs.join(' ')}`;
  }
}
exports.PnpmUI = PnpmUI;
_defineProperty(PnpmUI, "runtime", _ui().UIRuntime);
_defineProperty(PnpmUI, "dependencies", [_component().ComponentAspect]);
var _default = exports.default = PnpmUI;
