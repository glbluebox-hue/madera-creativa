...
export type VitestTesterOptions = {
  ...
  /**
   * Wall-clock window (ms) of zero run-progress after which a run is treated as
   * hung and force-terminated. Guards against tests stuck in synchronous code
   * that Vitest's in-worker `testTimeout` cannot interrupt (see hang-guard.ts).
   * Can also be overridden at runtime via the `BIT_VITEST_HANG_TIMEOUT` env var.
   * @default max(testTimeout + 5000, 15000)
   */
  hangTimeout?: number;
};
