...
export type TypeScriptCompilerOptions = {
    ...
    /**
     * Configuration for single-program compilation mode.
     *
     * When enabled, compiles all components in one TypeScript program instead of using
     * project references. This provides massive performance improvements (e.g., 42x faster)
     * especially for workspaces with circular dependencies.
     *
     * Use this mode when:
     * - You have circular dependencies between components
     * - Project references are causing performance issues
     * - All components can share the same tsconfig (with customizations via transformTsConfig)
     *
     * @example
     * ```typescript
     * singleProgramCompilation: {
     *   transformTsConfig: (tsconfig) => {
     *     // Remove environment-specific types to avoid conflicts
    ...
    singleProgramCompilation?: {
        /**
         * Function to transform the tsconfig before writing it to the capsule root.
         * This allows environment-specific customizations (e.g., removing conflicting types,
         * adjusting compiler options) to ensure compatibility across different environments.
         *
         * @param tsconfig - The tsconfig object to be modified (mutated in place)
         */
        transformTsConfig?: (tsconfig: Record<string, any>) => void;
    };
