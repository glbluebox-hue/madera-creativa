...
To create your own backend server for symphony install this component and implement the `BackendServer` interface.

```ts
import type { BackendServer } from '@bitdev/symphony.backends.backend-server';

export class MyServer implements BackendServer {
  name = 'express-server';
