...
export class DiffControlsModel {
    constructor(baseChannel, compareChannel) {
     ...
     */
    get hasSubscribers() {
        return (this.registry.getSubscribers(this._baseChannel).length > 0 ||
     ...
     */
    getValueForControl(controlId, source) {
        const baseState = this.registry.getState(this._baseChannel);
        const compareState = this.registry.getState(this._compareChannel);
        const defaultState = this.registry.getState('default');
        if (source === 'base') {
    ...
    get controls() {
        ...
        if (usingSameSource) {
            return baseDefs.map((def) => ({ ...def, source: 'common' }));
        }
     ...
     */
    updateControl(controlId, value, source) {
        if (source === 'common') {
    ...
    computeDiff(baseDefs, compareDefs) {
    ...
    areControlsEquivalent(a, b) {
        ...
        if (input === 'select' || input === 'multiselect') {
            if (!this.areOptionsEqual(a.options, b.options)) {
                return false;
            }
...
//# sourceMappingURL=diff-controls-model.js.map