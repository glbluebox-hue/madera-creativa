...
it('should show bubble when hovering on element with bit id', async () => {
    const { getByText, findByText } = render(_jsx(HoverHighlighter, { debounceSelection: debounceTime, children: _jsx(ButtonComponent, { children: "hover here" }) }));
...
it('should hide the highlight when hovering out of the element', async () => {
    const { getByText, findByText, queryByText } = render(_jsx(HoverHighlighter, { debounceSelection: debounceTime, children: _jsx(ButtonComponent, { children: "hover here" }) }));
    const rendered = getByText('hover here');
    act(() => {
        fireEvent.mouseOver(rendered);
    });
...
it('should hide the highlighter when moving the mouse away of it', async () => {
    const { getByText, queryByText, findByText } = render(_jsx(HoverHighlighter, { debounceSelection: debounceTime, children: _jsx(ButtonComponent, { children: "hover here" }) }));
    const rendered = getByText('hover here');
    // hover on target element:
    await act(async () => {
        fireEvent.mouseOver(rendered);
