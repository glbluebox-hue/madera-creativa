...
function getStringValue(node: Node) {
  ...
  if (
    ...
    node.quasis &&
    node.quasis.length === 1 && // if there is only one quasi, it means that the string is not interpolated (no variables in it)
...
export function getDependenciesFromMemberExpression(node: Node) {
  if (
...
export function getDependenciesFromCallExpression(node: Node) {
...
export function getSpecifierValueForImportDeclaration(specifier: any): Specifier {
  return {
