...
const checkInjection = (html: string, selector: string, expected: (attribs: Record<string, string>) => boolean) => {
  const $ = load(html);
  const element = $(selector).get(0) as { attribs: Record<string, string> } | undefined;
