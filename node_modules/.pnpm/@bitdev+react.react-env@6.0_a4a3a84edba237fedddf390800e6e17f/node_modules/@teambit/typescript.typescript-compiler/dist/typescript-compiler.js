...
class TypescriptCompiler {
    ...
    build(context) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.options.singleProgramCompilation) {
                try {
                    const componentsResults = yield this.runTsc(context.capsuleNetwork);
                    return {
    ...
    runTscBuild(network) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const seederIds = new Set(network.seedersCapsules.map((c) => c.component.id.toString()));
            const seederResults = componentsResults.filter((result) => seederIds.has(result.component.id.toString()));
    ...
    runTsc(network) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const rootDir = network.capsulesRootDir;
            yield this.writeRootTsconfigForNonProjectReferences(rootDir);
            const componentsResults = [];
            const componentErrorsMap = new Map();
            ...
            const reportDiagnostic = (diagnostic) => {
                ...
                const capsulePath = (_a = capsules.find(capsule => diagnostic.file.fileName.startsWith(capsule.path))) === null || _a === void 0 ? void 0 : _a.path;
                if (capsulePath) {
            ...
            const program = this.tsModule.createProgram(configParseResult.fileNames, configParseResult.options);
            ...
            yield this.copyCompiledFilesToCapsules(rootDir, capsules);
    ...
    parseRootTsconfig(rootDir, formatHost) {
        const rootTsconfigPath = path_1.default.join(rootDir, 'tsconfig.json');
        const configParseResult = this.tsModule.getParsedCommandLineOfConfigFile(rootTsconfigPath, undefined, this.tsModule.sys);
        if (!configParseResult) {
            throw new Error(`Failed to parse tsconfig.json at ${rootTsconfigPath}`);
        }
        if (configParseResult.errors.length > 0) {
            const formattedErrors = this.tsModule.formatDiagnosticsWithColorAndContext(configParseResult.errors, formatHost);
            throw new Error(`Errors in tsconfig.json:\n${formattedErrors}`);
        }
        return configParseResult;
    }
    handleDiagnosticError(diagnostic, formatHost) {
    ...
    writeRootTsconfigForNonProjectReferences(capsuleRoot) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const tsconfig = Object.assign({}, this.rawTsConfig);
            ...
            tsconfig.exclude = (0, lodash_1.uniq)([...newExclude, ...currentExclude]);
            // Apply custom tsconfig transformations if provided
            if ((_a = this.options.singleProgramCompilation) === null || _a === void 0 ? void 0 : _a.transformTsConfig) {
                this.options.singleProgramCompilation.transformTsConfig(tsconfig);
            }
            const tsconfigStr = this.stringifyTsconfig(tsconfig);
            yield fs_extra_1.default.writeFile(path_1.default.join(capsuleRoot, 'tsconfig.json'), tsconfigStr);
        });
    }
    copyCompiledFilesToCapsules(rootDir, capsules) {
        return __awaiter(this, void 0, void 0, function* () {
            const rootDistDir = path_1.default.join(rootDir, this.distDir);
            yield Promise.all(capsules.map((capsule) => __awaiter(this, void 0, void 0, function* () {
                const capsuleDistDir = path_1.default.join(capsule.path, this.distDir);
                const capsuleDirName = path_1.default.basename(capsule.path);
                // Copy only the files that belong to this specific component
                if (yield fs_extra_1.default.pathExists(rootDistDir)) {
                    // Get all files in the root dist directory
                    const allFiles = yield (0, glob_1.glob)('**/*', {
                        cwd: rootDistDir,
                        nodir: true,
                        absolute: false,
                    });
                    // Filter files that belong to this component (start with capsuleDirName/)
                    const componentFiles = allFiles.filter(file => file.startsWith(`${capsuleDirName}/`));
                    yield Promise.all(componentFiles.map((file) => __awaiter(this, void 0, void 0, function* () {
                        const srcPath = path_1.default.join(rootDistDir, file);
                        ...
                        const relativePath = file.startsWith(`${capsuleDirName}/`)
                            ...
                            : file;
                        const destPath = path_1.default.join(capsuleDistDir, relativePath);
                        // Special handling for source map files
                        if (file.endsWith('.js.map')) {
                            yield this.fixSourceMapPaths(srcPath, destPath, capsuleDirName);
                        }
                        else {
    ...
    fixSourceMapPaths(srcPath, destPath, capsuleDirName) {
        return __awaiter(this, void 0, void 0, function* () {
            const sourceMapContent = yield fs_extra_1.default.readFile(srcPath, 'utf8');
            ...
            yield fs_extra_1.default.outputFile(destPath, JSON.stringify(sourceMap));
        });
    }
    ...
    getAbsolutePathFormatDiagnosticsHost() {
        return {
            getCanonicalFileName: (p) => p,
