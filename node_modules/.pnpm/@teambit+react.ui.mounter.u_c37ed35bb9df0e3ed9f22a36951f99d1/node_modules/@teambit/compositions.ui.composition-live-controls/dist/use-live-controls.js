...
import { BROADCAST_READY_KEY, BROADCAST_UPDATE_KEY, BROADCAST_DESTROY_KEY, } from './composition-live-controls';
import { LiveControlsRegistry } from './live-controls-registry';
export function useLiveControls(channels) {
    ...
    const onEvent = useCallback((event) => {
        ...
        if (data.type === 'composition-live-controls:activate') {
            ...
            if (sourceWindow)
                liveControlsRegistry.setWindowChannel(sourceWindow, activateChannel);
        ...
        if (data.type === BROADCAST_READY_KEY) {
            ...
            liveControlsRegistry.registerReadyState(channel, data.payload.controls, data.payload.values, data.payload.timestamp, sourceWindow);
        }
    ...
    const setTimestamp = useCallback((ts) => {
        if (ts === 0) {
...
//# sourceMappingURL=use-live-controls.js.map