...
| **Contratación real de un plan (Stripe/checkout)** (deuda técnica, experiencia comercial de planes, 05/09/2026) | El botón "Elegir Basic/Pro/Premium" de `TarjetaPlan` no está conectado a ninguna pasarela de pago — solo confirma la elección por pantalla. Punto de integración marcado con un comentario en `elegir()` (`tarjeta-plan.tsx`). **PENDIENTE — checkout real, suscripciones, renovación, cambios de plan mensual↔anual, cancelación, gestión de tarjeta y webhooks de Stripe quedan todos fuera de esta fase.** |
| **Zona legal del registro (checkbox de Términos/Privacidad)** (deuda técnica, experiencia comercial de planes, 05/09/2026 — ligada a la entrada ya existente de aceptación legal más arriba) | `login-page.tsx` deja un comentario de código marcando dónde iría la casilla de aceptación, pero no la implementa: bloqueada por la Fase 2 legal pausada. **PENDIENTE — no resuelto por esta implementación, ni se pretendía.** |
| **Grid de planes con `auto-fit` en vez de un breakpoint fijo a 900px** (decisión técnica, experiencia comercial de planes, 05/09/2026) | `PaginaPlanes` usa `grid-template-columns: repeat(auto-fit, minmax(240px, 1fr))` (basado en el ancho del contenedor) en vez de una media query atada literalmente a los 900px del resto de la app, porque estas piezas usan estilos en línea (mismo patrón ya existente en `estado-trial.tsx`/`candado-plan.tsx`), no CSS Modules. El resultado visual es equivalente (3 columnas en desktop, 1 en móvil) pero el punto de corte real depende del contenedor (modal/pantalla completa), no de un valor de píxel fijo. No es un defecto, mismo criterio aceptado del resto del módulo; queda documentado por transparencia. |
...
- **Pagos**: fuera de esta fase por completo — sin Stripe, sin checkout, sin ningún estado simulado de "pago realizado". Ver la implementación completa de la interfaz comercial (registro → trial → planes) en el historial de decisiones más abajo — el botón "Próximamente" deshabilitado descrito aquí originalmente fue sustituido por un botón "Elegir…" habilitado que solo confirma la elección, nunca un pago.

### Historial de decisiones — Experiencia comercial de registro/trial/planes (05/09/2026)

Implementación completa de la interfaz visual de registro → trial → elección de plan, sobre la lógica de trial de 60 días ya existente (ver arriba), sin tocarla:

- **Catálogo comercial único** (`CATALOGO_PLANES`, `DESCUENTO_ANUAL_PORCENTAJE`, `calcularAhorroAnual`, `formatoPrecio` en `planes.ts`, frontend): nombres, lemas, precios mensual/anual, almacenamiento anunciado y funciones propias de BASIC/PRO/PREMIUM — único punto de verdad para toda la UI comercial (página de planes, banner del trial, pantalla de fin de trial). Precios oficiales EXACTOS como literales (nunca derivados por cálculo en coma flotante): BASIC 19 €/205,20 €, PRO 39 €/421,20 €, PREMIUM 59 €/637,20 € — descuento anual siempre del 10 %, verificado por test. Los GB anunciados son solo de presentación; el límite real vive en `almacenamiento-cuota.ts` (backend) y no se ha tocado.
- **`SelectorPeriodo`** (`selector-periodo.tsx`): un único interruptor mensual/anual, controla las tres tarjetas a la vez.
- **`TarjetaPlan`** (`tarjeta-plan.tsx`): tarjeta de un plan con acordeón "Ver todas las funciones" (`aria-expanded`, nunca modal ni página nueva) para el detalle completo. PRO lleva la etiqueta "RECOMENDADO" — decisión explícita del encargo, nunca "Más elegido". El botón "Elegir…" nunca se deshabilita ni simula un pago realizado: al pulsarlo solo confirma la elección por pantalla ("Has elegido X… la contratación con tarjeta estará disponible muy pronto"); el punto de integración real de pago queda marcado con un comentario en el código (`elegir()` en `tarjeta-plan.tsx`) — ver deuda técnica en § 8.
- **`PaginaPlanes`** (`pagina-planes.tsx`): ensambla selector + las tres tarjetas; componente de contenido puro (sin cabecera ni chrome de modal propios) para poder reutilizarse embebido tal cual en `PantallaTrialTerminado` (pantalla completa) y dentro de un modal (`.modalFondo`/`.modalCaja`) cuando se abre desde el banner del trial o desde "Mi perfil" (`presupuestos-prototype.tsx`) — nunca duplica la lógica de precios.
- **`aviso-marca-sketchup.tsx`** (nuevo, extraído de `tarjeta-modelo-3d.tsx`): única copia del texto/aviso de marca de SketchUp/Trimble en toda la app. En `TarjetaPlan`, el aviso se muestra siempre en la tarjeta PRO (independientemente de si el acordeón está expandido o no), para garantizar que nunca queda oculto detrás de un control adicional.
- **Banner del trial** (`estado-trial.tsx`, refinado, no reconstruido): mismo componente ya existente; texto actualizado a "Tu prueba de Madera Creativa Estudio continúa durante X días" + nuevo botón "Ver planes" que abre `PaginaPlanes`.
- **Pantalla de fin de trial** (`estado-trial.tsx`, refinado): el antiguo grid de 3 mini-tarjetas con botón deshabilitado se sustituyó por `<PaginaPlanes>` embebida directamente — mismo componente, mismos datos, con acordeón y CTA reales.
- **"Mi plan" en Mi Perfil** (`ajustes-perfil.tsx`): nueva sección con botón "Ver planes" (prop opcional `onVerPlanes`, omitida para el admin).
- **Registro** (`login-page.tsx`): tres insignias de confianza (60 días de prueba / Basic + Pro durante la prueba / Sin tarjeta) antes del formulario. La pantalla de "cuenta creada, revisa tu email" aclara explícitamente que el trial todavía no ha empezado. Zona legal (checkbox de Términos/Privacidad/consentimiento) dejada **sin implementar a propósito**, solo marcada con un comentario de código como punto de integración futura — la Fase 2 legal sigue pausada (`AUDITORIA-LEGAL-PRIVACIDAD.md` § 14) y el encargo prohibía explícitamente inventar un texto legal "provisional".
- **Bienvenida tras verificar el email** (`login-page.tsx`): sustituye la antigua confirmación de una línea por una pantalla con icono propio (SVG, un regalo con el mismo grosor de trazo que el resto de la app — nunca un emoji de sistema) y las 4 afirmaciones del encargo (trial empezado, Basic+Pro, sin tarjeta, datos se quedan).
- **Nada tocado de lo prohibido explícitamente** (§21 del encargo): autenticación, JWT, verificación de email, duración/lógica del trial (`calcularPlanEfectivo`, `iniciarTrialSiCorresponde`, `RUTAS_EXENTAS_BLOQUEO_PLAN`), backend de permisos, almacenamiento/R2, WebAuthn, presupuestos, portal, firma — todos sin cambios; confirmado también por los 518/518 tests de `presupuestos-service` sin ninguna modificación de backend en esta tarea.

---

**Última revisión de este documento: 05/09/2026** (añadidos 4 puntos abiertos de la implementación de cuota de almacenamiento por plan — ver § 8 —, la decisión definitiva de plan de Modelo 3D/SketchUp Desktop, la implementación completa de la prueba gratuita de 60 días, y la implementación completa de la interfaz comercial de registro/trial/planes — ver § 8 y § 9).
